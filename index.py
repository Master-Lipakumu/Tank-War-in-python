...
pygame.display.set_caption('Mona Technology Tank War')
...
def text_objects(text, color, font, size):
...
def message_to_screen(msg, color, y_displace=0, font=smallfont, size="small"):

    textSurf, textRect = text_objects(msg, color, font, size)
...
def text_to_button(msg, color, buttonx, buttony, 
                   buttonwidth, buttonheight, font, size="small"):
...
def game_controls():
    ...
    while gcont:
        ...
        message_to_screen("Controle", white, -100, size="large")
        message_to_screen("feu: Bar espace", wheat, -30)
        message_to_screen("Deplacer la tourelle: fleches haut et bas", wheat, 10)
        message_to_screen("Deplacer le reservoir: fleches gauche et droite", wheat, 50)
        message_to_screen("Appuyez sur D pour augmenter le % de puissance ET appuyez sur A pour réduire le % de puissance.", wheat, 140)
...
def pause():
    paused = True
    message_to_screen("Pause", introcolor, -100, size="large")
    message_to_screen("Appuyez sur C pour continuer à jouer ou sur Q pour quitter", wheat, 25)
    pygame.display.update()
    while paused:
        for event in pygame.event.get():
...
def fireShell(xy, tankx, tanky, turPos, gun_power, xlocation,
...
def e_fireShell(xy, tankx, tanky, turPos, gun_power, xlocation,
                 barrier_width, randomHeight, ptankx, ptanky):
...
def game_over():
    ...
    while game_over:
        ...
        gameDisplay.fill(black)
        message_to_screen("Jeu Fini", white, -100, size="large")
        message_to_screen("Vous etes Mort.", wheat, -30)

        button("Rejouer", 150, 500, 150, 50, wheat, light_green, action="play")
        button("Controles", 350, 500, 100, 50, wheat, light_yellow, action="controls")
        button("Quitter", 550, 500, 100, 50, wheat, light_red, action="quit")
...
def you_win():
    ...
    while win:
        ...
        gameDisplay.fill(black)
        message_to_screen("Vous etes Gagnant!", white, -100, size="large")
        message_to_screen("Félicitation!", wheat, -30)

        button("Rejouer", 150, 500, 150, 50, wheat, light_green, action="play")
        button("controle", 350, 500, 100, 50, wheat, light_yellow, action="controls")
        button("quitter", 550, 500, 100, 50, wheat, light_red, action="quit")
...
def gameLoop():
    ...
    while not gameExit:
        ...
        if gameOver == True:
            message_to_screen("Jeu Fini", red, -50, size="large")
            message_to_screen("Appuyez sur C Pour Rejouer ou Q pour Sortir", black, 50)
            pygame.display.update()
            while gameOver == True:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
...
def button(text, x, y, width, height, inactive_color,
            active_color, action=None, size="small"):
    ...
    text_to_button(text, black, x, y, width, height, font=font, size=size)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_c:
                    ...
                    quit()
        gameDisplay.fill(black)
        message_to_screen("Bienvenue!", introcolor, -100, font=largefont)
        message_to_screen("Tirez et détruisez le char ennemi", wheat, 15)
        message_to_screen("Avant qu'ils te détruisent.", wheat, 60)
        message_to_screen("Plus vous détruisez d'ennemis, plus ils deviennent difficiles.", wheat, 110)
        

        button("Jouer", 150, 500, 100, 50, wheat, light_green, action="play",size="vsmall")
        button("Controles", 350, 500, 100, 50, wheat, light_yellow, action="controls",size="vsmall")
        button("Quitter", 550, 500, 100, 50, wheat, light_red, action="quit",size="vsmall")
